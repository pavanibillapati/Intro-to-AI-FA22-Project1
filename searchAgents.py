    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(
                    ((updated_successor_pos, corners_left), action, 1))
    ...
    while corners_left:
        ...
        for corner in corners_left:
            ...
            track_distances.append(
                (util.manhattanDistance(present_state, corner), corner))
...
def foodHeuristic(state, problem):
    ...
    "dijkstra's"
    "for all of the remaining food, try every possible sequence of foods, calculate the distance of each sequence, then return the minimum length of all the sequences"
    distances = []
    foodList = foodGrid.asList()
 
    for food in foodList:
        "choose food as the start food"
        totalLength = util.manhattanDistance(position, food)
        "set up the first iteration"
        openList = list(foodList)
        openList.remove(food)
        closestFood = food
 
        while len(openList) > 0:
            "find the next closest food and cost"
            nextDistances = []
            nextClosestFood = []
 
            for nextFood in openList:
                nextDistances.append(util.manhattanDistance(closestFood, nextFood))
                nextClosestFood.append(nextFood)
            "record the nextClosestFood and record the distance between"
            totalLength += min(nextDistances)
            closestFood = nextClosestFood[nextDistances.index(min(nextDistances))]
            "update the openList"
            openList.remove(closestFood)
        "record the shortest distance of all of the sequences starting with the selected food"
        distances.append(totalLength)
    "in case there is no food at all or we have already reached our last fod"
    if len(distances) == 0:
        return 0
