        ...
        if not present_state in visited_nodes:
            ...
            for c_state, action, total_cost in problem.getSuccessors(present_state):
                ...
                if not c_state in visited_nodes:
                    update_fringe.push(
                        (c_state, set_actions + [action], total_cost))
    util.raiseNotDefined()
    ...
    while not update_fringe.isEmpty():
        ...
        if not present_state in visited_nodes:
            ...
            for c_state, action, total_cost in problem.getSuccessors(present_state):
                ...
                if not c_state in visited_nodes:
                    update_fringe.push(
                        (c_state, set_actions + [action], costs + total_cost), costs + total_cost)
    util.raiseNotDefined()
        ...
        if not present_state in visited_nodes:
            ...
            for c_state, action, total_cost in problem.getSuccessors(present_state):
                ...
                if not c_state in visited_nodes:
                    ...
                    cost_plus_heuristic = costs + \
                        total_cost + heuristic(c_state, problem)
                    update_fringe.push(
                        (c_state, set_actions + [action], costs + total_cost), cost_plus_heuristic)
    util.raiseNotDefined()
